...
void set_tag_config(uint8_t id) {
    MY_TAG_ID = id;
}
...
#define CALIB_TRUE_DIST 0.7071f  
...
bool calculate_tag_position(float *out_x, float *out_y) {
    vec2 valid_anchors[MAX_ANCHORS]; double valid_distances[MAX_ANCHORS]; int count = 0;
...
void ss_initiator_task_function(void *pvParameter) {
    ...
    static float sum_x = 0.0f, sum_y = 0.0f, sum_d0 = 0.0f, sum_d1 = 0.0f, sum_d2 = 0.0f, sum_d3 = 0.0f;
    ...
    while (1) {
        ...
        for (int a = 0; a < MAX_ANCHORS; a++) {
            ...
            tx_poll_msg[10] = (uint8)a; tx_poll_msg[2]++;          
            ...
            if (dwt_starttx(DWT_START_TX_IMMEDIATE | DWT_RESPONSE_EXPECTED) == DWT_SUCCESS) {
                ...
                if (dwt_read32bitreg(SYS_STATUS_ID) & SYS_STATUS_RXFCG) {
                    ...
                    if (rx_buffer[9] == 0xE1) {
                        uint32 t_round_t = dwt_readtxtimestamplo32(); uint32 t_round_r = dwt_readrxtimestamplo32();
                        uint32 t_reply_r = *(uint32*)&rx_buffer[10]; uint32 t_reply_t = *(uint32*)&rx_buffer[14];
                        int32 rtd_init = (int32)(t_round_r - t_round_t); int32 rtd_resp = (int32)(t_reply_t - t_reply_r);
                        ...
                        if (raw_dist > 0.05f && raw_dist < 100.0f) {
                            if (!is_calib_done[a]) {
                                raw_dist_sum[a] += raw_dist; calib_count[a]++;
                            ...
                            anchors_info[a].x = HARDCODED_ANCHOR_X[a]; anchors_info[a].y = HARDCODED_ANCHOR_Y[a];
                            anchors_info[a].dist = (anchors_info[a].is_valid) ? (anchors_info[a].dist * 0.7f + dist * 0.3f) : dist;
        ...
        if (calculate_tag_position(&tag_x, &tag_y)) {
            ...
            sum_x += tag_x; sum_y += tag_y;
            sum_d0 += d0; sum_d1 += d1; sum_d2 += d2; sum_d3 += d3;
            batch_count++;

            if (batch_count >= 5) {
                float avg_x = sum_x / 5.0f; float avg_y = sum_y / 5.0f;
                float avg_d0 = sum_d0 / 5.0f; float avg_d1 = sum_d1 / 5.0f;
                float avg_d2 = sum_d2 / 5.0f; float avg_d3 = sum_d3 / 5.0f;

                printf("{\"id\":%d,\"x\":%.2f,\"y\":%.2f,\"d\":[%.2f,%.2f,%.2f,%.2f]}\r\n", MY_TAG_ID, avg_x, avg_y, avg_d0, avg_d1, avg_d2, avg_d3);
                ...
                pkt.start_byte = '{'; pkt.id = MY_TAG_ID; pkt.x = avg_x; pkt.y = avg_y;

                if (xSemaphoreTake(radio_mutex, pdMS_TO_TICKS(50)) == pdTRUE) {
                    ble_raw_beacon_init(MY_TAG_ID);
                    for(int i = 0; i < 5; i++) {
                        pkt.seq = tag_seq++;
                ...
                batch_count = 0; sum_x = 0.0f; sum_y = 0.0f; sum_d0 = 0.0f; sum_d1 = 0.0f; sum_d2 = 0.0f; sum_d3 = 0.0f;
            }
        }
